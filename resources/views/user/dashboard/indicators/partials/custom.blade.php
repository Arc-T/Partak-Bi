<div class="tab-pane fade {{$tab_index == 2 ? 'show active' : '' }}" id="personal" role="tabpanel"
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            const url = "{{ Request()->sub_route == 'province' ? "Province" : "City" }}";

            let select = '';

            if ($('select[name="mdf[]"]').length) {
                select = 'select[name="mdf[]"]';
            } else if ($('select[name="city[]"]').length) {
                select = 'select[name="city[]"]';
            } else if ($('select[name="province[]"]').length) {
            ...
            $.ajax({
                url: "{{ Request()->sub_route == 'province' ? route('getProvinces') : route('getCities') }}",
                type: "GET",
                data: parameters,
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    $(select).append(`<option value="0"> خطا در دریافت اطلاعات </option>`);
